...
.footer .footerBox {
	display: grid;
	grid-template-columns: repeat(6, 110px); /* 2개의 동일한 폭의 칸으로 나눔 */
...
<script type="text/javascript">
	...
	function tableSearch() {
		$.ajax({
			...
			url : '/jabusim_Project/admin/examResultTableSearch/?name=' + name
					+ '&examName=' + examName + '&examSubject=' + examSubject
					+ '&examStart=' + examStart + '&examEnd=' + examEnd
					+ '&resultStart=' + resultStart + '&resultEnd=' + resultEnd
					+ '&examType=' + examType + '&resultStatus=' + resultStatus
	...
	function updateModel(examResultBeanList) {
		...
		for (var i = 0; i < examResultBeanList.length; i++) {
			...
			dynamicHtml += "<tr>"
					+ "<th><input type='checkbox' class='checkList' onclick='updateCounter()' value='"
					+ examResultBean.examResult_idx
					+ "'></th>"
					+ "<td><span>"
					+ (i + 1)
					+ "</span></td>"
					+ "<td><span>"
					+ examResultBean.user_name
					+ "</span></td>"
					+ "<td><span>"
					+ examResultBean.user_id
					+ "</span></td>"
					+ "<td><span>"
					+ examResultBean.exam_name
					+ "</span></td>"
					+ "<td><span>"
					+ examResultBean.exam_subject
					+ "</span></td>"
					+ "<td><span>"
					+ examResultBean.exam_type
					+ "</span></td>"
					+ "<td><span>"
					+ examResultBean.exam_date
					+ "</span></td>"
					+ "<td><span>"
					+ examResultBean.exam_resultDate
					+ "</span></td>"
					+ "<td><span>"
					+ examResultBean.examResult_status
					+ "</span></td>"
					+ "<td><span>"
					+ examResultBean.examResult_processStatus
					+ "</span></td>" + "</tr>";
		}
	...
	function deleteList() {
		...
		if (checkedCount > 0) {
			$
					.ajax({
						type : 'GET',
						url : '/jabusim_Project/admin/examResultTableDelete/?checkedList='
								+ checkedList,
	...
	function updateList(status) {
		...
		if (checkedCount > 0) {
			$
					.ajax({
						type : 'GET',
						url : '/jabusim_Project/admin/examResultTableUpdate/?checkedList='
								+ checkedList + '&status=' + status,
						success : function(result) {
							console.log("성공");
							alert("수정했습니다");
							resetCheck();
	...
	function processList(status) {
		...
		if (checkedCount > 0) {
			$
					.ajax({
						type : 'GET',
						url : '/jabusim_Project/admin/examResultTableProcess/?checkedList='
								+ checkedList + '&status=' + status,
						success : function(result) {
							console.log("성공");
							alert("수정했습니다");
...
<body>
				...
				<div class="dashBoard">
					...
					<ul class="nav nav-tabs">
						<li class="nav-item"><a class="nav-link"
							href="${root}adminPage/examManage/examManage">시험관리</a></li>
						<li class="nav-item"><a class="nav-link"
							href="${root}adminPage/examManage/examPlaceManage">시험장소</a></li>
						<li class="nav-item"><a class="nav-link"
							href="${root}adminPage/examManage/examReceiptManage">시험접수</a></li>
						<li class="nav-item"><a class="nav-link active"
							aria-current="page" href="#"
							style="border: 1px solid #dfe1e5; border-bottom: 1px solid transparent">시험결과</a>
						</li>
					</ul>
						...
						<form action="">
							<table class="searchTable">
								...
								<tr>
									...
									<td class="searchArticle">
										<div class="searchReq">
											<input type="date" id="examStartInput" /> ~ <input
												type="date" id="examEndInput" />
											<!-- max="2077-06-20" -->
										</div>
									</td>
								</tr>
								<tr>
									...
									<td class="searchArticle">
										<div class="searchReq">
											<input type="date" id="resultStartInput" /> ~ <input
												type="date" id="resultEndInput" />
											<!-- max="2077-06-20" -->
										</div>
									</td>
								</tr>
								<tr>
									...
									<td class="searchArticle">
										<div class="searchReq">
											<input type="checkbox" value="입력불가" name="resultStatusInput" />
											입력불가
										</div>
										<div class="searchReq">
											<input type="checkbox" value="미입력" name="resultStatusInput" />
											미입력
										</div>
										<div class="searchReq">
											<input type="checkbox" value="합격예정" name="resultStatusInput" />
											합격예정
										</div>
										<div class="searchReq">
											<input type="checkbox" value="불합격예정" name="resultStatusInput" />
											불합격예정
										</div>
										<div class="searchReq">
											<input type="checkbox" value="합격" name="resultStatusInput" />
											합격
										</div>
										<div class="searchReq">
											<input type="checkbox" value="불합격" name="resultStatusInput" />
											불합격
										</div>
									</td>
								</tr>
								<tr>
									...
									<td class="searchArticle">
										<div class="searchReq">
											<input type="checkbox" value="미처리" name="processStatusInput" />
											미처리
										</div>
										<div class="searchReq">
											<input type="checkbox" value="처리가능" name="processStatusInput" />
											처리가능
										</div>
										<div class="searchReq">
											<input type="checkbox" value="처리완료" name="processStatusInput" />
											처리완료
										</div>
									</td>
								</tr>
								<tr>
									...
									<td class="searchArticle">
										<div class="searchReq">
											<input type="text" id="examSubjectInput" class="form-control"
								...
								<tr>
									...
									<td class="searchArticle">
										<div class="searchReq">
											<input type="text" id="examNameInput" class="form-control"
												placeholder="search" aria-label="Username"
												aria-describedby="basic-addon1">
										</div>
									</td>
								...
								<c:forEach items="${allExamResultBeans}" var="examResultBean"
									...
									<tr>
										<th><input type="checkbox" class="checkList"
											...
											value="${examResultBean.examResult_idx}" /></th>
	...
	<div class="footer" id="footer">
		<div class="footerBox">
			...
			<div class="footerButton">
				<button type="button" onclick="updateList('합격')"
					class="btn btn-danger">합격</button>
			</div>
			<div class="footerButton">
				<button type="button" onclick="updateList('불합격')"
					class="btn btn-danger">불합격</button>
			</div>
			<div class="footerButton">
				<button type="button" onclick="processList('처리완료')"
					class="btn btn-danger">처리완료</button>
			</div>
		</div>
	</div>
	<script
